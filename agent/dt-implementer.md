...
### Backend implementation (Zetta Reports Backend)
- Routes live in `src/routes/{resource}.ts` — export a default `Router()`
- Mount routes in `src/index.ts` under `/api` prefix
- Use `dbManager.getPool(dbName)` to get SQL Server connection pools
- **Always use parameterized queries** — `pool.request().input(name, value).query(...)` — never concatenate SQL
- Use the project's logger: `createLogger('ModuleName')` — never `console.log`
- Validate inputs with Zod (installed but apply it to new routes)
- All queries are read-only (SELECT) unless explicitly approved for writes
- Follow existing patterns in `src/routes/reports.ts` for new report endpoints
- Register new databases in `src/index.ts` via `dbManager.registerFromEnv([...names])`
