...
### Backend-specific scenarios *(Zetta Reports Backend)*
- API endpoint request/response validation (Express 5 routes under `/api`)
- Database connection management (`dbManager.getPool()` success and failure paths)
- SQL query parameterization (ensure no SQL injection vectors)
- Input validation via Zod schemas (query params: `db`, `table`, filters)
- Error handling for database connectivity failures and timeouts
- HTTP status codes (200 success, 400 bad request, 500 server error, 503 degraded)
- INFORMATION_SCHEMA introspection queries (tables, columns)
- Health check endpoints (service health, DB connectivity with latency)
- CORS configuration (allowed origins in dev vs production)
- Missing or invalid `?db=` query parameter handling
