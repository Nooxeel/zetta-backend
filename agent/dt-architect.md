...
### SQL Server Database Safety *(Zetta Reports Backend)*

**This backend connects to external SQL Server databases that are NOT owned by us.** These are production databases from client systems (SoMee hosted). All interactions MUST be strictly READ-ONLY unless explicitly authorized.

**Mandatory rules:**

1. **Default to SELECT-only queries** — no INSERT, UPDATE, DELETE, DROP, ALTER, or TRUNCATE unless the task explicitly requires write access and is approved by the team
2. **Always use parameterized queries** — `pool.request().input(name, value).query(...)` — never concatenate user input into SQL strings
3. **Connection pools are lazy** — registered via `dbManager.registerFromEnv()`, connected on first `getPool()` call
4. **Each database is identified by name** — routes receive `?db=<name>` and resolve via `dbManager.getPool(name)`
5. **New databases require env var registration** — follow the `DB_{NAME}_SERVER/DATABASE/USER/PASSWORD/PORT` pattern

**Risk assessment checklist for any database-related change:**

| Check | Pass criteria |
|-------|---------------|
| No write operations | Zero INSERT/UPDATE/DELETE/DDL unless explicitly approved |
| Parameterized queries | All user-supplied values go through `.input()` — zero string concatenation |
| Connection pool limits | max 10 connections, idle timeout 30s, query timeout 30s |
| Error handling | Failed queries return proper error responses, don't leak connection details |
| INFORMATION_SCHEMA only | Schema introspection uses INFORMATION_SCHEMA views, not system tables |

**If a developer proposes ANY write operation to a client database, flag it as HIGH RISK and require:**
- Written justification for why a write is necessary
- Explicit approval from the team lead
- Confirmation that the target database permits writes from our service account
